...
#define TabelView{
    ...
    m_view->setItemDelegateForColumn(0,alignmentdelegate);
    ...
    //connect
    connect(header,SIGNAL(checkBoxClicked(bool)),this,SLOT(doDebug(bool)));
    connect(this,SIGNAL(ChangeAllCheck(bool)),m_model,SLOT(ckeckALL(bool)));
...
#define AddDataButtons{
    ...
    lbRemove->setAlignment( Qt::AlignCenter);
    panelLayout->addWidget( lbRemove, 0, 1 );
...
void ModelViewDemoWidget::doDebug(bool value)

{
    ...
    emit ChangeAllCheck(value);
    qDebug()<<"ChangeALLcheck signal emit";
}
// ********************************************************************************
...
void PersonsModel::ckeckALL(bool value)
{
    qDebug()<<m_persons[0].value(SURNAME);

    qDebug()<<"checkALL slot";
